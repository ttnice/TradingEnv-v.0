...
class Env(gym.Env):
    ...
    def _next_observation(self):
        ...
        position = np.zeros((60, ))
        position[:3] = self.position
    ...
    def _take_action(self, action):
        ...
        if buy_profit < current_price or sell_profit > current_price:
            ...
            if action_type == 0:
                ...
                self._position_trend(holding=True)
            elif action_type == 1:
                # Buying
                if self.trend == 1:
                    # Confirming buying trend
                    self.reward = temp_reward
                    self._position_trend(holding=False)
                else:
                    if temp_reward > 1:
    ...
    def _position_trend(self, holding):
        if holding:
            # Holding trend
            hold_decrease = self.position[self.trend % 3] / 2
            self.position[0] += hold_decrease  # Holding trend confirmed
            self.position[self.trend % 3] -= hold_decrease  # Variant trend unconfirmed
        else:
            # Variant trend
            hold_decrease = self.position[self.trend % 3] / 2
            self.position[0] -= hold_decrease  # Holding trend unconfirmed
            self.position[self.trend % 3] += hold_decrease  # Variant trend confirmed

    def _current_price(self):
        current_price = random.uniform(self.df['Low'][self.current_step], self.df['Low'][self.current_step])
